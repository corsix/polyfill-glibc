...
#include "assembler.h"
#include "../../build/aarch64/assembled_gen.h"
...
#define LOCAL_FLAG_ALREADY_POPULATED 0x01
#define LOCAL_FLAG_BTI_HEAD 0x02
#define LOCAL_FLAG_IS_PLT 0x04
...
static void parse_extern_sym(renamed_symbol_t* dst, const uint16_t* head) {
  ...
  const uint64_t* relocs = (const uint64_t*)(head + 4);
...
static void add_sym_and_deps(polyfiller_t* self, uint32_t token) {
  if (uuht_set_if_absent(&self->added, token, 1)) return;
  ...
  if (n_reloc) {
    ...
    for (uint32_t i = 0; i < n_reloc; ++i) {
      add_sym_and_deps(self, relocs[i] >> 32);
    }
  }
...
static uint32_t polyfiller_add(void* self, const char* name, uint8_t* stt) {
  ...
  if (token) {
    ...
    add_sym_and_deps((polyfiller_t*)self, token);
  }
  return token;
}
...
static bool polyfiller_add_fixup_for_dt_relr(void* self_) {
  polyfiller_t* self = (polyfiller_t*)self_;
  add_sym_and_deps(self, token_for(_polyfill_DT_RELR_apply));
  return true;
}
...
#define polyfill_cie "\x10\0\0\0\0\0\0\0\01zR\0\01\x78\x1e\01\x1b\x0c\x1f\0"
#define polyfill_fde_len 17
  ...
  uint64_t cfi_base_v = var->addr = erw_alloc(self->erw, erw_alloc_category_v_r_near_x, 4, size_fde);
  uint64_t hdr_v = 0;
  ...
  for (local_info_t* sym = self->addrs, *end = sym + self->added.count; sym < end; ++sym) {
    ...
    uint32_t n_cfi = head[3];
    if (flags & ASM_SYM_FLAG_EXTERN) {
      if (!(sym->flags & LOCAL_FLAG_IS_PLT)) continue;
      n_code = PLT_SIZE;
      n_cfi = 0;
    } else if (!n_code) {
      continue;
...
static void populate_init_variables(polyfiller_t* self, uint32_t num_init_fn) {
  ...
  var->addr = erw_alloc(self->erw, erw_alloc_category_v_r_near_x, 4, num_init_fn * 4);
  if (!self->erw->retry) {
    ...
    var = self->addrs + uuht_lookup_or(&self->added, token_for(_polyfill_init_array_count), ~0u);
    var->flags |= LOCAL_FLAG_ALREADY_POPULATED;
    memcpy(erw_view_v(self->erw, var->addr), &num_init_fn, sizeof(num_init_fn));
  }
...
static void populate_fini_variables(polyfiller_t* self, uint32_t num_fini_fn) {
  ...
  if (num_fini_fn <= 1) {
    ...
    var->addr = erw_alloc(self->erw, erw_alloc_category_v_r_near_x, 8, 8);
  ...
  var->addr = erw_alloc(self->erw, erw_alloc_category_v_r_near_x, 8, num_fini_fn * 8);
  if (!self->erw->retry) {
    ...
    var = self->addrs + uuht_lookup_or(&self->added, token_for(_polyfill_fini_array_count), ~0u);
    var->flags |= LOCAL_FLAG_ALREADY_POPULATED;
    memcpy(erw_view_v(self->erw, var->addr), &num_fini_fn, sizeof(num_fini_fn));
  }
...
static void populate_dt_relr_variables(polyfiller_t* self) {
  ...
  erw_relocs_add_early(self->erw, DT_RELA, R_AARCH64_IRELATIVE, dummy_v, 0, fn->addr);
...
static void populate_regular_symbol(polyfiller_t* self, local_info_t* sym, const uint16_t* head) {
  ...
  for (uint32_t i = 0; i < n_reloc; ++i) {
    uint64_t r = relocs[i];
    local_info_t* target = self->addrs + uuht_lookup_or(&self->added, r >> 32, ~0u);
    uint64_t target_addr = target->addr;
    uint32_t where = (uint32_t)r >> 8;
    operand_class_other_t kind = (operand_class_other_t)(r & 0xff);
    switch (kind) {
    case operand_class_other_rel16:
    case operand_class_other_rel21:
    case operand_class_other_rel28:
      if (target->flags & LOCAL_FLAG_BTI_HEAD) {
        target_addr += 4;
      }
      // fallthrough
    ...
    case operand_class_other_rel32:
      if (!encode_reloc(kind, dst + where, target_addr - (sym->addr + where))) {
        FATAL("TODO: reloc out of range %llu -> %llu", (long long unsigned)(sym->addr + where), (long long unsigned)target_addr);
      }
      break;
    case operand_class_other_phantom_ref:
      break;
    default:
      FATAL("TODO: bad runtime reloc kind %u", kind);
    }
  }
}
...
static void write_plt(erw_state_t* erw, uint64_t addr, uint64_t target) {
  uint32_t* dst = (uint32_t*)erw_view_v(erw, addr);
  uint64_t delta = target - addr;
  if (S_OK_64(delta, 21)) {
    // adr x16, ...
    *dst++ = 0x10000010 ^ ((delta & 3) << 29) ^ ((delta << 3) & 0xffffe0);
  } else {
    // adrp x16, ...
    uint64_t t_p = target & ~(uint64_t)0xfff;
    uint64_t a_p = addr & ~(uint64_t)0xfff;
    delta = (int64_t)(t_p - a_p) >> 12;
    if (!S_OK_64(delta, 21)) {
      FATAL("Displacement too large even for adrp");
    }
    *dst++ = 0x90000010 ^ ((delta & 3) << 29) ^ ((delta << 3) & 0xffffe0);
    // add x16, x16, ...
    delta = target - t_p;
    *dst++ = 0x91000210 ^ (delta << 10);
  }
  // ldr x17, [x16]
  *dst++ = 0xf9400211;
  // br x17
...
#define NNE_INSTANTIATE "aarch64/polyfiller_nne.h"
...
static void polyfiller_finished_adding(void* self_) {
  ...
  if (self->seen_func_flags) {
    ...
    if (any_cfi) {
      ...
      if (!erw_phdrs_find_first(self->erw, PT_GNU_EH_FRAME)) {
        ...
        add_sym_and_deps(self, token_for(_polyfill_cfi_data));
        ...
        add_sym_and_deps(self, token_for(_polyfill_init_cfi_strong));
      } else {
        ...
        add_sym_and_deps(self, token_for(_polyfill_init_cfi_weak));
      }
    }
    ...
    if (self->seen_func_flags & ASM_SYM_FLAG_INIT) {
      ...
      if (erw_dhdrs_has(self->erw, DT_INIT, NULL)) {
        ...
        add_sym_and_deps(self, token_for(_polyfill_DT_INIT));
      } else {
        // Added as a new DT_INIT.
        add_sym_and_deps(self, token_for(_polyfill_init));
      }
      if (erw_phdrs_is_entry_callable(self->erw)) {
        // Wrapper around entry point.
        add_sym_and_deps(self, token_for(_polyfill_entry));
      }
    }
    ...
    if (self->seen_func_flags & ASM_SYM_FLAG_FINI) {
      ...
      add_sym_and_deps(self, token_for(_polyfill_DT_FINI));
    }
  }
  ...
  for (uint32_t i = 0, j = 0; i <= self->added.mask; ++i) {
    ...
    if (slot) {
      ...
      sym->addr = i; // Putting this here temporarily; will become addr in the next pass.
    }
  }
  ...
  for (local_info_t* sym = self->addrs, *end = sym + self->added.count; sym < end; ++sym) {
    ...
    if (flags & ASM_SYM_FLAG_EXTERN) {
      ...
      v = erw_alloc(self->erw, erw_alloc_category_v_rx, CODE_ALIGN, n_code);
      if (n_code >= 4) {
        uint32_t n_reloc = head[1];
        const uint64_t* relocs = (const uint64_t*)(head + 4);
        const uint32_t* src = (const uint32_t*)(relocs + n_reloc);
        if ((*src & 0xffffff3f) == 0xd503241f) {
          sym->flags |= LOCAL_FLAG_BTI_HEAD;
        }
      }
    ...
    uint32_t table_index = (uint32_t)sym->addr;
    sym->addr = v;
    self->added.table[table_index] = (uint32_t)self->added.table[table_index] | ((uint64_t)own_idx << 32);
  }
  if (dsym_to_ext.count) {
    ...
    qsort(dsym_to_ext.table, dsym_to_ext.mask + 1ull, sizeof(dsym_to_ext.table[0]), sort_u64_asc);
    for (uint32_t i = 0; i <= dsym_to_ext.mask; ++i) {
      uint64_t slot = dsym_to_ext.table[i];
      if (!slot) continue;
      local_info_t* sym = self->addrs + (slot >> 32);
      if (!sym->addr) {
        uint32_t sidx = uuht_unhash((uint32_t)slot);
        ...
        erw_relocs_add(self->erw, DT_RELA, R_AARCH64_GLOB_DAT, sym->addr, sidx, 0);
      }
      // Extern functions are transformed from being a function pointer variable
      // to being a PLT stub function.
      uint64_t v = erw_alloc(self->erw, erw_alloc_category_v_rx, CODE_ALIGN, PLT_SIZE);
      if (v) write_plt(self->erw, v, sym->addr);
      sym->addr = v;
      sym->flags |= LOCAL_FLAG_IS_PLT;
    }
  }
  ...
  for (local_info_t* sym = self->addrs, *end = sym + self->added.count; sym < end; ++sym) {
    ...
    if (sym->flags & LOCAL_FLAG_ALREADY_POPULATED) continue;
...
const polyfiller_fns_t polyfiller_fns_aarch64 = {
